...
import (
	...
	"github.com/CyrusCorrell0/MD-TUI/ui"
)
...
func (m *FilePickerModel) View(width, height int) string {
	header := ui.HeaderStyle.Render(fmt.Sprintf("Mol TUI  [%s]", m.cwd))
	help := ui.MutedStyle.Render("up/down or j/k navigate   enter select   q quit")
	if m.empty {
		empty := ui.MutedStyle.Render("No .pdb files found in current directory.")
