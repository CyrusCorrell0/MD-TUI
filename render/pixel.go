...
import (
	...
	"github.com/CyrusCorrell0/MD-TUI/pdb"
)
...
func writeAnsiRGBPair(sb *strings.Builder, fr, fg, fb, br, bg, bb uint8) {
	...
	writeByteDecimal(sb, fr); sb.WriteByte(';')
	writeByteDecimal(sb, fg); sb.WriteByte(';')
	writeByteDecimal(sb, fb); sb.WriteString("m\x1b[48;2;")
	writeByteDecimal(sb, br); sb.WriteByte(';')
	writeByteDecimal(sb, bg); sb.WriteByte(';')
	writeByteDecimal(sb, bb); sb.WriteByte('m')
}
